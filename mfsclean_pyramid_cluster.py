...
for n in nsb:
	...
	name = 'cleanMFQ_flag1_SB'+spwr+'_it'+str(its)+'_wp'+str(wp)+'_facets'+str(fac)+'_npix'+str(npix)
	pclean(vis, spw=spwr, field='0', ftmachine='wproject', alg='clark', mode='continuum', majorcycles=10, wprojplanes=wp, facets=fac, imagename=name, niter=its, interactive=False, imsize=[npix, npix], cell=[pixsize, pixsize])
	...
	msg1 =  'Clean finished for SB %s, RMS = %f, pixel size = %d, image width = %d degrees.' % (spwr, rms[-1], pixsize, npix*pixsize/3600)
	print msg1
	tf = time.time()
	rt.append(tf-ts)
	msg2 = 'Run time: '+str(rt[-1])
	print msg2
