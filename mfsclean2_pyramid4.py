...
for n in range(sbstart, sbend):
	print n
	ts = time.time()
	spwr = str(n)+'~'+str(n+3)
	print 'Starting clean for SB'+spwr+'...'
	name = 'cleanMFQ_flag2_SB'+spwr+'_it'+str(its)+'_wp'+str(wp)+'_facets'+str(fac)+'_npix'+str(npix)
	clean(vis, spw=spwr, field='0', mode='mfs', gridmode='widefield', wprojplanes=wp, facets=fac, imagename=name, niter=its, interactive=False, imsize=[npix, npix], cell=ceil(synthbeamasec[n+3]))
